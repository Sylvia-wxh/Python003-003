...
#读取处理猫眼主页面
# maoyan = open('file:///C:/Users/Swu1/GitHub%20Code/Python003-003/week10/UsageCollect/phone.html')
# print(maoyan)

path = os.path.abspath(os.path.dirname(sys.argv[0]))
# htmlfile = open(path+'/phone.htm', encoding='utf-8')
# htmlhandle = htmlfile.read()
# soup = bs(htmlhandle, 'lxml')
bs_data = bs(open(path+'/shampoo.html', encoding='utf-8'), features='html.parser')

# main_response = requests.get(maoyan, header=header)
# selector = lxml.etree.HTML(main_response.text)
# # bs_data = bs(main_response.text, 'html.parser')

alist = []
# phones = selector(response=bs_data).xpath('//li[@data-position<11]')
for tags in bs_data.find_all('h5', attrs = {'class': 'feed-block-title'}, limit=10):
    for tag in tags.find_all('a'):
        alist.append(tag.get('href'))
print(alist)
#取child page后缀，形成链接
# alist = []
